...
const els={
  ...
  details:$('#imageDetails'),lua:$('#luaPreview'),badge:$('#assignedBadge'),copy:$('#copyLua'),dl:$('#downloadLua'),test:$('#testLua'),
  ...
  favoriteOnly:$('#favoriteOnly'),templateModal:$('#templateSaveModal'),templateModalClose:$('#templateModalClose'),templateModalCancel:$('#templateModalCancel'),templateModalSave:$('#templateModalSave'),templateName:$('#templateName'),templateCategory:$('#templateCategory'),templateAuthor:$('#templateAuthor'),templateTag:$('#templateTag'),templateDesc:$('#templateDesc'),templateRuleSummary:$('#templateRuleSummary'),simRun:$('#runSimulation'),simReset:$('#resetSimulation'),simFound:$('#simFoundImages'),simVariables:$('#simVariables'),simElapsed:$('#simElapsed'),simState:$('#simulationState'),simLog:$('#simulationLog')
};
const simulation={calls:0,time:0,variables:{},imageEnabled:{},started:false,sequence:0};
...
function selectImage(name){state.selected=name;if(!state.scripts[name])state.scripts[name]=[];resetSimulation(false);renderAll();persist()}
function renderAll(){renderImages();renderSelection();renderTemplates()}
function renderSelection(){
  const img=selectedImage(),active=!!img;els.copy.disabled=els.dl.disabled=els.test.disabled=els.saveTemplate.disabled=!active;
...
function simulationValue(value){const s=String(value??'').trim();if(/^[-+]?\d+(\.\d+)?$/.test(s))return Number(s);if(s==='true')return true;if(s==='false')return false;if(s==='nil'||s==='')return null;return s}
function parseSimulationVariables(){const out={};for(const part of els.simVariables.value.split(',')){const i=part.indexOf('=');if(i<1)continue;const key=part.slice(0,i).trim();if(/^[A-Za-z_]\w*$/.test(key))out[key]=simulationValue(part.slice(i+1))}return out}
function simulationFound(){return new Set(els.simFound.value.split(',').map(x=>x.trim()).filter(Boolean))}
function simulationLog(kind,text){simulation.sequence++;if(simulation.sequence===1)els.simLog.innerHTML='';const row=document.createElement('div');row.className=`simulation-row-v6 ${kind}`;row.innerHTML=`<span>${String(simulation.sequence).padStart(2,'0')}</span><b>${html(kind)}</b><code>${html(text)}</code>`;els.simLog.append(row);els.simLog.scrollTop=els.simLog.scrollHeight}
function compareSimulation(a,operator,b){return {'==':a===b,'~=':a!==b,'>':a>b,'>=':a>=b,'<':a<b,'<=':a<=b}[operator]??false}
function simulationCondition(condition,found){const type=condition?.type||'current',p=condition?.params||{};if(type==='current')return {result:true,text:'현재 이미지 액션 실행'};if(type==='found'||type==='notfound'){const hit=found.has(p.image||'');return {result:type==='found'?hit:!hit,text:`ImageSearch(${p.image||'대상 없음'}) = ${hit?1:0}`}}if(type==='enabled'||type==='disabled'){const on=simulation.imageEnabled[p.image]??state.images.find(x=>x.name===p.image)?.enabled??false;return {result:type==='enabled'?on:!on,text:`${p.image||'대상 없음'} ${on?'활성':'비활성'}`}}if(type==='variable'){const a=simulation.variables[p.varName]??0,b=simulationValue(p.value),result=compareSimulation(a,p.operator||'==',b);return {result,text:`${p.varName||'변수'}(${JSON.stringify(a)}) ${p.operator||'=='} ${JSON.stringify(b)}`}}if(type==='multi'){const result=String(p.expression||'').split('|').some(group=>group.split('&').map(x=>x.trim()).filter(Boolean).every(name=>found.has(name)));return {result,text:`MultiImage(${p.expression||''})`}}return {result:false,text:`${conditionDefs[type]?.label||type} · 모의 입력 미지원`}}
function runSimulationAction(action){const p=action.params||{},label=actionDefs[action.type]?.label||action.type;if(action.type==='setVar'){simulation.variables[p.varName||'result']=simulationValue(p.value);simulationLog('변수',`${p.varName||'result'} = ${JSON.stringify(simulation.variables[p.varName||'result'])}`);return}if(action.type==='addVar'||action.type==='subVar'){const key=p.varName||'result',amount=Number(p.amount)||0;simulation.variables[key]=Number(simulation.variables[key]||0)+(action.type==='addVar'?amount:-amount);simulationLog('변수',`${key} = ${simulation.variables[key]}`);return}if(action.type==='toggleVar'){const key=p.varName||'result';simulation.variables[key]=simulation.variables[key]?0:1;simulationLog('변수',`${key} = ${simulation.variables[key]}`);return}if(action.type==='enable'){simulation.imageEnabled[p.image||'']=String(p.enabled)!=='false';simulationLog('ImageMax',`EnableImage(${simulation.imageEnabled[p.image||'']}, ${p.image||'대상 없음'})`);return}if(action.type==='print')simulationLog('Print',p.text||'');else if(action.type==='printVar')simulationLog('Print',`${p.label||p.varName||'변수'}: ${JSON.stringify(simulation.variables[p.varName]??0)}`);else simulationLog('예정 명령',`${label}${Object.keys(p).length?` · ${Object.entries(p).map(([k,v])=>`${k}=${v}`).join(', ')}`:''} · 실제 실행 안 함`)}
function runSimulationActions(actions){for(const action of actions||[])runSimulationAction(action)}
function runSimulationRule(rule,index,found){simulationLog('블록',`${index+1}. ${ruleSummary(rule)}`);if(rule.kind==='counter'){const key=rule.varName||'image_count';simulation.variables[key]=Number(simulation.variables[key]||0)+1;const hit=simulation.variables[key]>=Math.max(1,Number(rule.threshold)||1);simulationLog('조건',`${key} = ${simulation.variables[key]} / ${rule.threshold||1} → ${hit?'참':'거짓'}`);if(hit){runSimulationActions(rule.actions);simulation.variables[key]=0}return}if(rule.kind==='retry'){const key=rule.varName||'recheck_count',hit=found.has(rule.image||'');simulationLog('검색',`ImageSearch(${rule.image||'대상 없음'}) = ${hit?1:0}`);if(hit){runSimulationActions(rule.foundActions);simulation.variables[key]=0}else{simulation.variables[key]=Number(simulation.variables[key]||0)+1;simulationLog('상태',`${key} = ${simulation.variables[key]} / ${rule.attempts||1}`);if(simulation.variables[key]>=Math.max(1,Number(rule.attempts)||1)){runSimulationActions(rule.exhaustedActions);simulation.variables[key]=0}}return}if(rule.kind==='elapsed'){const key=rule.varName||'elapsed_timer',started=simulation.variables[key]??simulation.time,hit=simulation.time-started>=Math.max(0,Number(rule.seconds)||0);if(simulation.variables[key]===undefined)simulation.variables[key]=started;simulationLog('조건',`${simulation.time-started}초 / ${rule.seconds||0}초 → ${hit?'참':'거짓'}`);if(hit){runSimulationActions(rule.actions);simulation.variables[key]=simulation.time}return}if(rule.kind==='once'){const key=rule.varName||'once_done',hit=!simulation.variables[key];simulationLog('조건',`${key} = ${simulation.variables[key]||0} → ${hit?'최초 실행':'이미 실행됨'}`);if(hit){runSimulationActions(rule.actions);simulation.variables[key]=1}return}if(rule.kind==='cooldown'){const key=rule.varName||'cooldown_time',last=Number(simulation.variables[key]||0),hit=simulation.time-last>=Math.max(0,Number(rule.seconds)||0);simulationLog('조건',`${simulation.time-last}초 / ${rule.seconds||0}초 → ${hit?'참':'거짓'}`);if(hit){runSimulationActions(rule.actions);simulation.variables[key]=simulation.time}return}if(rule.kind==='randomFound'||rule.kind==='bestMatch'){const candidates=(rule.images||[]).filter(x=>found.has(x));if(!candidates.length){simulationLog('검색','발견된 후보 없음');return}let pick=candidates[0];if(rule.kind==='bestMatch')pick=candidates.map(name=>state.images.find(x=>x.name===name)).filter(Boolean).sort((a,b)=>b.accuracy-a.accuracy).find(x=>x.accuracy>=Number(rule.minAccuracy||0))?.name||'';simulationLog('예정 명령',pick?`${pick} ${rule.kind==='randomFound'?'선택':'최고 정확도 선택'} → 클릭 · 실제 실행 안 함`:'최소 정확도를 넘는 후보 없음');return}const check=simulationCondition(rule.condition,found);simulationLog('조건',`${check.text} → ${check.result?'참':'거짓'}`);runSimulationActions(check.result?rule.then:rule.else)}
function renderSimulationState(){els.simState.textContent=`호출 ${simulation.calls}회 · 가상 시간 ${Number(simulation.time.toFixed(2))}초 · 변수 ${Object.keys(simulation.variables).length}개`}
function resetSimulation(log=true){Object.assign(simulation,{calls:0,time:0,variables:{},imageEnabled:{},started:false,sequence:0});els.simLog.innerHTML='<div class="simulation-empty">“1회 실행”을 누르면 블록별 조건과 실행 예정 명령이 표시됩니다.</div>';renderSimulationState();if(log)toast('모의 실행 상태를 초기화했습니다.')}
function runSimulation(){if(!state.selected)return toast('이미지를 먼저 선택하세요.');if(!simulation.started){simulation.variables={...parseSimulationVariables()};simulation.started=true}simulation.calls++;simulation.time+=Math.max(0,Number(els.simElapsed.value)||0);simulation.sequence=0;els.simLog.innerHTML='';simulationLog('호출',`${state.selected} · ${simulation.calls}회차 · 가상 시간 ${Number(simulation.time.toFixed(2))}초`);const found=simulationFound();for(const [index,rule] of (state.scripts[state.selected]||[]).entries())runSimulationRule(rule,index,found);if(!(state.scripts[state.selected]||[]).length)simulationLog('안내','실행할 블록이 없습니다.');simulationLog('완료','실제 장치 입력 없이 ImageMax 제어권으로 반환');renderSimulationState()}
...
els.test.onclick=()=>{activateWorkPane('test');runSimulation()};els.simRun.onclick=runSimulation;els.simReset.onclick=()=>resetSimulation(true);
